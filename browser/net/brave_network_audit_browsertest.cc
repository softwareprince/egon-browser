...
// Loads egon://welcome first to simulate a first run and then loads another
...
IN_PROC_BROWSER_TEST_F(BraveNetworkAuditTest, BasicTests) {
  ...
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), GURL("egon://welcome")));
  ...
  // Finally, load egon://rewards and enable Brave Rewards.
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), GURL("egon://rewards")));
  ASSERT_TRUE(EnableBraveRewards());
  WaitForTimeout(kMaxTimeoutPerLoadedURL);

  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), GURL("egon://wallet")));
  WaitForTimeout(kMaxTimeoutPerLoadedURL);
